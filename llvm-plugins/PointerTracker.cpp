...
STATISTIC(DangRegisterPtrCallDot,
          "Number of store instructions instrumented for dot function");
...
struct PointerTracker : public FunctionPass {
    ...
    static bool shouldProcessFunction(const char *name) {
        /* SPEC CPU2006 povray: Cone->apex_radius is considered a pointer,
         * causing a pointer lookup on a double value */
        if (strcmp(name, "_ZN3pov17Compute_Cone_DataEPNS_13Object_StructE") ==
            0)
            return false;
        if (strcmp(name, "_ZN3povL17Transform_QuadricEPNS_13Object_StructEPNS_"
                         "16Transform_StructE") == 0)
            return false;
        if (strcmp(name, "_ZN3povL12Plane_NormalEPdPNS_13Object_StructEPNS_"
                         "10istk_entryE") == 0)
            return false;
        ...
        for (inst_iterator I = inst_begin(F), E = inst_end(F); I != E; ++I) {
            ...
            */
            ...
            if (StoreInst *Store = dyn_cast<StoreInst>(Inst)) {
                ...
                /* Comment out this condition, if stack objects has to be
                 * tracked.
                 */
                ...
                */
                ...
                if (ValueType &&
                    isa<FunctionType>(ValueType->getElementType())) {
                ...
                if (isa<ConstantPointerNull>(object_addr)) {
                    DEBUG_MSG(errs() << "Store : Object pointer with constant "
                                        "null value \n");
                    continue;
                }
  ...
  private:
    ...
    bool instrumentVectorStore(Value *ptr_addr, Value *obj_addr,
                               IRBuilder<> &B) {
        DEBUG_MSG(errs() << "Vector Store instrumentation \n");
        assert(obj_addr->getType()->isVectorTy() &&
               "Store has to be vector type ");
        return true;
    }
    ...
    bool instrumentStore(Module *M, Function &F, Value *ptr_addr,
                         Value *obj_addr, IRBuilder<> &B) {
        ...
        if (M->getDataLayout().getTypeStoreSizeInBits(objTy) ==
             ...
             */
            /*
             * If Object type is 64 bits but not pointer or integer type. It is
             * mostly of double type. Thus, explicitely cast it to i64.
             */
            ...
            M->getOrInsertFunction(functionName, VoidTy, IntPtrTy, IntPtrTy);
    ...
    void AccumulateStackStores(const Instruction *AI,
                               std::set<const Instruction *> &stackStores) {
    ...
    bool isGlobalPointer(Value *V) {
        ...
        if (BitCastInst *BC = dyn_cast<BitCastInst>(V)) {
            ...
            return isGlobalPointer(GEP->getPointerOperand());
                       ...
                       V)) { /* TODO: Remove load instruction */
            if (AA->pointsToConstantMemory(LI->getOperand(0))) {
                DEBUG_MSG(errs() << "Global constant load \n");
                return true;
            }
        } else if (SelectInst *SI = dyn_cast<SelectInst>(V)) {
            return (isGlobalPointer(SI->getTrueValue()) &&
                    isGlobalPointer(SI->getFalseValue()));
        }
        return false;
    }
    ...
    bool isPointerOperand(Value *V) {
        ...
        if (LoadInst *LI = dyn_cast<LoadInst>(V)) {
            ...
            if (ConstantExpr *CE = dyn_cast<ConstantExpr>(LoadPtr)) {
                /*
                 * Note: Be careful with constant expression. getAsInstruction()
                 * creates Dangling reference. TODO: remove this and operate
                 * only on CE
                 */
