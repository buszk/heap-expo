...
cl::opt<bool>
    FixedCompression("METALLOC_FIXEDCOMPRESSION",
                     cl::desc("Enable fixed compression for METADATA"),
                     cl::init(false));
cl::opt<unsigned long> MetadataBytes ("METALLOC_METADATABYTES", cl::desc("Number of METADATA bytes"), cl::init(8)/*,
    cl::values(
        ...
        clEnumVal(8, "8"),
        clEnumVal(16, "16"))*/);
cl::opt<bool> DeepMetadata("METALLOC_DEEPMETADATA",
                           cl::desc("Enable multi-level METADATA"),
                           cl::init(true));
...
unsigned long
SafetyManager::GetStaticAllocaAllocationSize(const AllocaInst *AI) {
...
bool SafetyManager::IsSafeStackAlloca(const Value *AllocaPtr,
                                      unsigned long AllocaSize) {
    ...
    while (!WorkList.empty()) {
        ...
        for (const Use &UI : V->uses()) {
            ...
            case Instruction::Load: {
                if (!IsAccessSafe(UI, DL->getTypeStoreSize(I->getType()),
            ...
            case Instruction::Store: {
                ...
                if (!IsAccessSafe(
                        UI, DL->getTypeStoreSize(I->getOperand(0)->getType()),
                        AllocaPtr, AllocaSize))
                    return false;
                break;
            }
            ...
            case Instruction::Invoke: {
                ...
                // LLVM 'nocapture' attribute is only set for arguments whose
                // address is not stored, passed around, or used in any other
                // non-trivial way. We assume that passing a pointer to an
                // object as a 'nocapture readnone' argument is safe.
                // FIXME: a more precise solution would require an
                // interprocedural analysis here, which would look at all uses
                // of an argument inside the function being called.
...
void SafetyManager::AccumulateSideEffects(
    const Value *AllocaPtr, std::set<const Instruction *> &SideEffects) {
    SmallPtrSet<const Value *, 16> Visited;
    SmallVector<const Value *, 8> WorkList;
    WorkList.push_back(AllocaPtr);
...
void SafetyManager::AccumulateUnsafeSideEffects(
    const Value *AllocaPtr, unsigned long AllocaSize,
    std::set<std::pair<const llvm::Instruction *, const llvm::Value *>>
        &UnsafeSideEffects) {
    SmallPtrSet<const Value *, 16> Visited;
    SmallVector<const Value *, 8> WorkList;
    WorkList.push_back(AllocaPtr);
...
void SafetyManager::AccumulateSafeSideEffects(
    const Function *F, std::set<const Instruction *> &SafeSideEffects) {
    ...
    for (const Instruction &I : instructions(F)) {
        if (auto AI = dyn_cast<AllocaInst>(&I)) {
            ...
            AccumulateUnsafeSideEffects(AI, GetStaticAllocaAllocationSize(AI),
                                        UnsafeSideEffects);
        }
        if (auto CI = dyn_cast<CallInst>(&I)) {
            if (CI->getCalledFunction() &&
                (CI->getCalledFunction()->getName().equals("_Znwm") ||
                 CI->getCalledFunction()->getName().equals("_Znam") ||
                 CI->getCalledFunction()->getName().equals("malloc")) &&
                ...
                AccumulateUnsafeSideEffects(CI, Size, UnsafeSideEffects);
                       ...
                       CI->getCalledFunction()->getName().equals(
                           "__errno_location")) {
                AccumulateSideEffects(CI, AllSideEffects);
            }
        }
    ...
    std::set_difference(AllSideEffects.begin(), AllSideEffects.end(),
                        RelevantUnsafeSideEffects.begin(),
                        RelevantUnsafeSideEffects.end(),
...
void SafetyManager::AccumulateSafeSideEffects(
    const Module *M, std::set<const Instruction *> &SafeSideEffects) {
    std::set<const Instruction *> AllSideEffects;
    std::set<std::pair<const Instruction *, const Value *>> UnsafeSideEffects;
